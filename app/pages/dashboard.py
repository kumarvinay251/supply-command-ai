...
def render(role: str) -> None:
    ...
    with k3:
        if role in ("Operations Manager", "CFO"):
            ...
            st.markdown(
                """
                <div style='padding:16px; border:1px solid #333; border-radius:6px;
    ...
    st.caption("Auto-evaluated on load · No query required · Powered by validated SQL templates")
    ...
    if not alerts:
        ...
        if high_alerts:
            st.error(f"🔴 {len(high_alerts)} HIGH severity alert(s) require immediate attention")
        if medium_alerts:
            st.warning(f"🟡 {len(medium_alerts)} MEDIUM severity alert(s) flagged for review")
        ...
        for alert in alerts:
            ...
            <div style="border-left: 4px solid {severity_color};
                ...
                <div style="font-weight:700; font-size:14px; margin-bottom:4px;">
                    ...
                    <span style="font-size:11px; color:{severity_color};
                                 ...
                                 border-radius:3px; border:1px solid {severity_color};">
                        {alert['severity']}
                    </span>
                </div>
            ...
            # ── Expander: detail table + recommendations ──────────────────
    ...
    st.markdown("---")
    # ── END PROACTIVE RISK ALERTS ──────────────────────────────────────

    # ═══════════════════════════════════════════════════════════════════════════
    #  ROW 2 — DELAY TREND + SUPPLIER PERFORMANCE
    # ═══════════════════════════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    #  ROW 3 — ROI PROGRESSION (financial roles only)
    # ═══════════════════════════════════════════════════════════════════════════
    ...
    if role != "Demand Planner":
        ...
        # Demand Planner sees a restricted notice instead of financial charts
        st.info(
